...
def validate_number(number):
    return isinstance(number, int) and 0 < number <= 999999
...
def lambda_handler(event, context):
    try:
        ...
        response = players_table.get_item(Key={'nickname': nickname})
        if 'Item' in response:
            return response(409, error="Nickname già in uso. Scegli un altro.")
